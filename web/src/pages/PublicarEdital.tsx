...
export default function PublicarEdital() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMensagem("Não foi possível publicar o edital");
    }
  }

  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <Sidebar paginaAtiva="publicar-edital" />
      <div style={{ flex: 1, padding: 20, maxWidth: 480 }}>
        <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 16 }}>Publicar edital</div>
        ...
        <form onSubmit={handleSubmit} style={{ background: "#fff", padding: 20, borderRadius: 10, border: "0.5px solid var(--cor-borda)" }}>
          ...
          <Campo label="Descrição">
            <textarea name="descricao" rows={3} style={inputStyle} />
          </Campo>

          <Campo label="Prêmio (opcional)">
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 13, color: "#666" }}>R$</span>
              <input name="premio" type="number" min={0} step="any" placeholder="1500,00" style={inputStyle} />
            </div>
          </Campo>

          <Campo label="Prazo de inscrição">
            <input name="prazo_inscricao" type="date" required style={inputStyle} />
          </Campo>
          ...
          {temLimiteVagas && (
            <Campo label="Número de vagas">
              <input name="vagas" type="number" min={1} style={inputStyle} />
            </Campo>
          )}
